...
class DiggerMove:
    def __init__(self, direction:str, distance:int, color:str):
        self.direction = color[-2]

        self.distance = int(color[2:-2],base=16) // 4000
        self.color = color
    ...
    def estimate_hole(moves: List["DiggerMove"]) -> Tuple[int, int]:
        ...
        width:int = dmax["0"]+dmax["2"]
        height:int = dmax["1"]+dmax["3"]
        print(f"Estimated: {width=}, {height=}  product={width*height}")
        return width,height
    ...
    def dig(ground:List[List[str]], moves:List["DiggerMove"], startc:int, startr:int):
        ...
        width = len(ground[0])
        height = len(ground)
        print(f"Adjusted: {width=}, {height=}  product={width*height}")
